...
class TCPSender {
  private:
    ...
    uint64_t _next_seqno{0}; // first unsent byte

    size_t _remaining_size{0}; // remaining space for sending new bytes to the receiver

    size_t _bytes_in_flight{0}; // number of bytes which are not acknowledged
    
    std::queue<TCPSegment> _outstanding_segments{}; // segments which are not acknowledged, stored in a queue
    
    uint64_t _current_ackno{0}; // latest (largest) ackno received from the receiver
