...
class AsyncNetworkInterface : public NetworkInterface {
   ...
   public:
    ...
    //! - If type is IPv4, pushes to the `datagrams_out` queue for later
...
class Router {
    ...
    //! Send a single datagram from the appropriate outbound interface to the
    //! next hop, as specified by the route with the longest prefix_length that
    //! matches the datagram's destination address.
    void route_one_datagram(InternetDatagram& dgram);

   public:
    //! Add an interface to the router
    //! \param[in] interface an already-constructed network interface
    //! \returns The index of the interface after it has been added to the
    //! router
