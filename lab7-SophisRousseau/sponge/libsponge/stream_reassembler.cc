...
void StreamReassembler::push_substring(const string& data,
    ...
    if (index == first_unassembled) {
        ...
        string s;
        for (size_t i = first_unassembled + len;
             i < first_unaccepted && _stream.find(i) != _stream.end(); i++) {
            s += _stream[i];
            _stream.erase(i);
        }
        _output.write(s);
    } else if (index > first_unassembled) {
        for (size_t i = index; i < index + data.size() && i < first_unaccepted;
             i++) {
            _stream[i] = data[i - index];
        }
    } else if (index + data.size() > first_unassembled) {
        return push_substring(data.substr(first_unassembled - index),
                              first_unassembled, eof);
    }
    ...
    if (_eof &&
        _output.bytes_written() ==
            _eof_index) {  // 可能乱序到达，因此最后一个报文段到达也不一定整个报文都到达了
        _output.end_input();  // all the bytes are pushed into the stream
    }
}
